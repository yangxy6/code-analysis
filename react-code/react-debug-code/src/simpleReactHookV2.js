...
function mountReducer(initState, hook) {
    // 1. 处理保存hook
    if (isMount) {
        // mount时需要生成hook对象
        hook = { //hook是链表
...
function updateReducer(reducer, hook) {
    // 1. 处理保存hook
    ...
    if (hook.queue.pending) {
        ...
        do {
            ...
            if (reducer) {
                // reducer useReducer和useState唯一的不同
...
function useState(initState) {
    ...
    let hook;
    if (isMount) {
        hook = mountReducer(initState, hook)
    } else {
        // 第一个参数null 代表没有reducer
        hook = updateReducer(null, hook)
    }
    let baseState = hook.memoizedState
...
function useReducer(reducer, initialState) {
    ...
    let hook;
    if (isMount) {
        hook = mountReducer(initialState, hook)
    } else {
        hook = updateReducer(reducer, hook)
    }
    let baseState = hook.memoizedState
...
function App() {
    ...
    const [reducerState, dispatch] = useReducer(reducer, { init: 0 })
